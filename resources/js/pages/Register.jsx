            ...
            if(error.response) {
                if(error.response.status == 429) {
                    ...
                    if(error.response.data?.errors) {
                        ...
                        if(errors?.name) {
                            setErrorName(errors.name.map(el => {
                                return { message: el };
                            }));
                        }
                        if(errors?.surname) {
                            setErrorSurname(errors.surname.map(el => {
                                return { message: el };
                            }));
                        }
                        if(errors?.email) {
                            setErrorEmail(errors.email.map(el => {
                                return { message: el };
                            }));
                        }
                        if(errors?.profile_picture) {
                            setErrorProfilePicture(errors.profile_picture.map(el => {
                                return { message: el };
                            }));
                        }
                        if(errors?.username) {
                            setErrorUsername(errors.username.map(el => {
                                return { message: el };
                            }));
                        }
                        if(errors?.password) {
                            setErrorPassword(errors.password.map(el => {
                                return { message: el };
                            }));
                        }
                        if(errors?.confirm_password) {
                            setErrorConfirmPassword(errors.confirm_password.map(el => {
                                return { message: el };
                            }));
                        }
                    }
            ...
            <CardContent>
                <form onSubmit={handleSubmit}>
                    <FieldGroup>
                        ...
                        <Field data-invalid={error_profile_picture.length > 0}>
                            ...
                            <Input
                                ...
                                aria-invalid={error_profile_picture.length > 0}
                                type="file"
                                ></Input>
                            <FieldDescription>Allowed types: jpeg, png, webp</FieldDescription>
